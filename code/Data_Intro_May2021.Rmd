...
ggplot(islet_prey2, aes(x = Order, y = percent, fill = Island)) +
  ...
  facet_wrap(~prod_level)
```

```{r}
ggplot(islet_prey2, aes(x = Order, y = percent, fill = prod_level)) +
  geom_boxplot(space = 0, position = position_dodge(preserve = "single")) +
  theme_bw() +
  coord_flip() +
  labs(y = "Frequency (percentage of samples)", x = "Prey Order") +
  scale_fill_manual(values = c("#80cdc1", "#8c510a")) 
```
...
Could I also incorporate the DNA data from more spiders + the potential prey items I picked up somehow to answer these questions? Maybe top predators aren’t shifting their diets all that much but food chain length is increasing in the middle of the food web?
  ...
  - this is challenging because i only have prey items from a few islands, and not at the ends of the spectrum. 
