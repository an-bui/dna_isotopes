...
source(here("code",
            "source",
            "source_preyisotopes.R"))

source(here("code", 
            "source",
            "source_isotope_niches.R"))
```
...
spider_iso %>%
  mutate(Mass_mg = 100*Mass_g) %>%
ggplot(aes(x = Mass_mg, y = d13C, color = prod_level)) +
  geom_point(size = 3) +
  #geom_smooth(method = "lm", se = F) +
  scale_color_manual(values = c("#80cdc1",
                               "#bf812d")) +
...
```{r}
prey_iso %>%
  mutate(Group = case_when(Order %in% c("Hemiptera") ~ "Herbivores",
                           Order %in% c("Diptera", 
                                   "Lepidoptera",
                                   "Hymenoptera",
                                   "Orthoptera") ~ "Omnivores",
                           Order %in% c("Araneae") ~ "Predators",
                           TRUE ~ NA_character_)) %>%
  mutate(Order = factor(Order, 
                        levels = c("Diptera", 
                                   ...
                                   "Orthoptera",
                                   "Araneae"))) %>%
ggplot(aes(x = Order, y = d15N_c, fill = prod_level)) +
  geom_boxplot(size = 1, alpha = 0.6) +
  geom_point(aes(color = prod_level), 
             position=position_jitterdodge()) +
...
```{r}
prey_iso %>%
ggplot(aes(x = prod_level, y = d15N_c, fill = prod_level)) +
  geom_boxplot(size = 1, alpha = 0.6) +
  geom_point(aes(color = prod_level), 
             position=position_jitterdodge()) +
  labs(fill = "Productivity level", y = expression({delta}^15*N~ ('\u2030'))) +
...
```{r, eval = FALSE}
library(lme4)
model2 <- lmer(d15N_c ~ prod_level + (1|Order), data = prey_iso)
summary(model2)
simulateResiduals(model2, plot= T)
plot(allEffects(model2))
dotplot(ranef(model2))
#this model is a bit ugly
model4 <- lmer(d13C ~ prod_level + (1|Order), data = prey_iso)
summary(model4)
